...
- **Validation:** local migration replay, pgTAP RLS/integrity/search tests, Vitest server-query tests, Chromium development and production-server E2E, lint, typecheck, DB lint, and production build are supported. Independent adversarial review and its narrow correction pass are complete locally; Cloud migration/application and hosted validation remain pending.

SPEC-002 remains active pending Cloud migration/application, hosted validation, and formal closure; this section records implemented and independently reviewed local state without closing its lifecycle.
