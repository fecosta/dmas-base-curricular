...
The Next.js application and Supabase identity foundation are implemented, locally tested, and have passed independent security/RLS review. The Google OAuth application flow is implemented and locally tested at the orchestration and provider-independent authorization layers. The SPEC-001 identity migration has been applied to the intended Supabase Cloud project and its RLS/grants/ownership posture inspected there; Google OAuth provider configuration, a real hosted Google round trip, hosted OTP validation, and Vercel deployment remain unverified. Sections describing curriculum, workflow, search, Storage, and Resend remain the approved architecture for later slices, not implemented functionality.

**Current implementation:** `docs/DECISIONS.md` (D-028) is implemented in application code as Google OAuth through Supabase Auth (primary), with Email OTP retained as fallback. §7 describes the authentication architecture and §18 records implementation detail. Hosted Google provider configuration and end-to-end validation remain outstanding. This does not change the authorization model in §8.
...
This architecture is implemented in the application. Hosted Google provider configuration and a real provider round trip remain deployment validation work.
