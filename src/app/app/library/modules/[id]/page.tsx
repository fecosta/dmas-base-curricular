  ...
  return <main id="contenido" className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
    ...
    <div className="mt-10 grid gap-12 lg:grid-cols-[1fr_0.55fr]">
      <div className="space-y-12">
        ...
        <section aria-labelledby="notes-title"><h2 id="notes-title">Notas docentes</h2>{curriculumModule.teachingNotes.length ? <div className="mt-4 space-y-4">{curriculumModule.teachingNotes.map((note) => <article key={note.id} className="rounded-xl bg-white/65 p-5"><h3 className="font-bold">{note.title}</h3>{note.programTopicId && <p className="mt-1 text-xs font-bold uppercase tracking-wide text-[#c84b31]">Programa: {topicName.get(note.programTopicId)}</p>}{note.text && <p className="mt-3 text-slate-700">{note.text}</p>}{note.materials.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{note.materials.map((material) => <Link key={material.id} href={`/app/library/references/material/${material.id}`} prefetch={false} className="rounded-full bg-[#e7c76e]/40 px-3 py-1 text-sm font-bold text-[#173f3a] no-underline">{material.title}</Link>)}</div>}{note.sourceUrl && <a href={note.sourceUrl} rel="noreferrer" target="_blank" className="mt-3 inline-block font-bold">Abrir fuente externa</a>}<PublishedAttachments attachments={note.attachments} /></article>)}</div> : <p className="mt-3 text-slate-600">No hay notas docentes publicadas.</p>}</section>
      </div>
